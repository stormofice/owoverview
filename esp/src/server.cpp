...
void WebServer::run() const
{
    ...
    server->on("/init_partial", HTTP_GET, [this](AsyncWebServerRequest* request) {
        const auto job = EpdJob{EpdJobKind::InitPartial};
        xQueueSendToBack(this->job_queue, &job, portMAX_DELAY);
        request->send(200, "text/plain", "Initializing display...");
    });
    ...
    server->on("/upload_image_partial", HTTP_POST, [this](AsyncWebServerRequest* request) {
        if (!request->_tempObject) {
            return request->send(400, "text/plain", "Invalid request, nothing uploaded");
        }


        if (!request->hasParam("rect")) {
            return request->send(400, "text/plain", "Invalid request, no rect param");
        }
        const auto rect = request->getParam("rect")->value();
        // x, y, w, h
        uint64_t aux[4]  =  {0};
        if (sscanf(rect.c_str(), "%llu,%llu,%llu,%llu", &aux[0], &aux[1], &aux[2], &aux[3]) != 4) {
            return request->send(400, "text/plain", "Invalid request, invalid rect param");
        }

        printf("Upload end\r\n");

        const auto partial_upload = static_cast<PartialUpload *>(request->_tempObject);
        const auto job = EpdJob {EpdJobKind::DisplayPartial, partial_upload->data, partial_upload->acc_size, aux};
    ...
    server->on("/direct_image_partial", HTTP_POST, [this](AsyncWebServerRequest* request) {
        if (!request->_tempObject) {
            return request->send(400, "text/plain", "Invalid request, nothing uploaded");
        }
        ...
        const auto partial_upload = static_cast<PartialUpload *>(request->_tempObject);
        const auto job = EpdJob {EpdJobKind::DirectPartial, partial_upload->data, partial_upload->acc_size, aux};
